...
# ============================================================
# Swift static analysis (type graph + call graph + metrics)
# ============================================================
...
def analyze_snapshot(snapshot_dir: str,
                     ...
                     debt_dir: str = None,
                     top_dir: str = None) -> list[dict]:
    """
    Returns list of cluster dicts:
    {
      "cluster": [Type1, Type2, ...] (sorted),
      "size": int,
      "DebtMag": float,
      "NormDebt": float,
      "RankScore": float
    ...
    # ---------------------------------------
    # VALIDATION: type graph integrity
    # ---------------------------------------
    assert set(G.nodes()) == set(types.keys()), \
        f"Type graph node mismatch: {snapshot_name}"

    # ---------------------------------------
    # VALIDATION: type graph edge integrity
    # ---------------------------------------
    for u, v in G.edges():
        assert u in types and v in types, \
            f"Type graph contains invalid edge: {u}->{v} in {snapshot_name}"
    ...
    # ---------------------------------------
    # DEBUG EXPORT: complete dependency call graph
    # ---------------------------------------
    if callgraph_dir:
        nx.write_gexf(
            call_g,
    ...
    # stored properties per type (names)
    props_by_type = {}
    for t, body in types.items():
        props = set()
    ...
    for t in type_names:
        ...
        LCOT_T[t] = float(compute_LCOT(t, funcs, props_by_type.get(t, set())))
    ...
    # RFT_T
    for t in type_names:
        RFT_T[t] = float(compute_RFT(t, funcs_by_type, call_g))

    # ED_T: based on weakly connected component in full graph
    ...
    for t in type_names:
        comp = comp_map.get(t, {t})
        n = len(comp)
        if n <= 1:
            ED_T[t] = 0.0
            continue
        # count directed edges among comp in original G
        e = 0
        for u in comp:
            for v in G.successors(u):
                if v in comp:
                    e += 1
        ED_T[t] = float(e) / float(n * (n - 1))

    # CY_T: SCC membership size > 1
    ...
    for scc in sccs:
        ...
        for n in scc:
            scc_size[n] = size
    for t in type_names:
        CY_T[t] = 1.0 if scc_size.get(t, 1) > 1 else 0.0

    # -----------------------------
    # Thresholds (hybrid rules)
    # -----------------------------
    # Distributions
    ...
    D_ED = [ED_T[t] for t in type_names]

    Q3_CC = third_quartile(D_CC)
    Q3_WMT = third_quartile(D_WMT)

    tau = {}
    # CC baseline 10, but adjust upward to Q3 if Q3 > 10
    tau["CC_T"] = max(10.0, Q3_CC)
    # WMT baseline 20, but adjust upward to Q3 if Q3 > 20
    tau["WMT_T"] = max(20.0, Q3_WMT)
    # LCOT purely distributional
    tau["LCOT_T"] = third_quartile(D_LCOT)
    # CBT, RFT distributional
    tau["CBT_T"] = third_quartile(D_CBT)
    tau["RFT_T"] = third_quartile(D_RFT)
    # ED distributional
    tau["ED_T"] = third_quartile(D_ED)
    # CY binary
    tau["CY_T"] = 0.0
    ...
    # -----------------------------
    metric_keys = ["CC_T", "WMT_T", "LCOT_T", "CBT_T", "RFT_T", "ED_T", "CY_T"]
    if weights is None:
        w = {k: 1.0 / len(metric_keys) for k in metric_keys}
    else:
        # normalize to sum to 1 over provided keys; fill missing with 0
    ...
    # Excess per type
    # -----------------------------
    def excess_val(metric_name: str, t: str) -> float:
        val = {
            "CC_T": CC_T[t],
            "WMT_T": WMT_T[t],
            "LCOT_T": LCOT_T[t],
        ...
        return max(0.0, float(val) - float(tau[metric_name]))

    total_excess = {}
    for t in type_names:
        total_excess[t] = sum(w[m] * excess_val(m, t) for m in metric_keys)

    V_D = {t for t in type_names if total_excess[t] > 0.0}

    if not V_D:
        return [], []

    # -----------------------------
    # Debt-induced subgraph + clusters
    # -----------------------------
    Gd = G.subgraph(V_D).copy()

    # -------------------------
    # VALIDATION: induced property
    # -------------------------
    expected_edges = {
        (u, v)
        for u, v in G.edges()
        if u in Gd.nodes() and v in Gd.nodes()
    }
    assert set(Gd.edges()) == expected_edges, \
        f"Debt subgraph is not properly induced: {snapshot_name}"

    graph_summaries.append(
        summarize_graph(Gd, "DebtSubgraph", snapshot_name)
    )

    # -------------------------
    # VALIDATION: node integrity
    # -------------------------
    assert set(Gd.nodes()).issubset(set(G.nodes())), \
        f"Debt subgraph contains invalid nodes: {snapshot_name}"

    # -------------------------
    # VALIDATION: edge bound sanity
    # -------------------------
    n = len(Gd.nodes())
    max_edges = n * n  # directed case
    assert len(Gd.edges()) <= max_edges, \
        f"Debt subgraph edge count invalid: {snapshot_name}"

    if debt_dir:
        nx.write_gexf(
            Gd,
            os.path.join(debt_dir, f"{snapshot_name}_debt_subgraph.gexf")
        )

    UGd = Gd.to_undirected(as_view=True)

    clusters = []

    for comp in nx.connected_components(UGd):

        sub = Gd.subgraph(comp)
        assert nx.is_connected(sub.to_undirected()), \
            f"Cluster not connected: {snapshot_name}"

        comp = sorted(comp)
        size = len(comp)

        debtmag = float(sum(total_excess[t] for t in comp))
        normdebt = float(debtmag / size) if size > 0 else 0.0
        rankscore = float(debtmag / math.sqrt(size)) if size > 0 else 0.0

        # -----------------------------
        # Cluster metric aggregates
        # -----------------------------
        cluster_metrics = {}
        ...
        for m in metric_keys:
            ...
            cluster_metrics[m] = float(np.mean(vals)) if vals else 0.0

        # -----------------------------
        # Cluster excess contributions
        # -----------------------------
        cluster_excess = {}
        ...
        for m in metric_keys:
            vals = []
            for t in comp:
                vals.append(excess_val(m, t))
            cluster_excess[m] = float(np.sum(vals))
        ...
        clusters.append({
            ...
            "excess": cluster_excess,
            "thresholds": tau
        })

    # Sort: RankScore desc, then DebtMag desc, then canonical tie-break by smallest type name
    def _cluster_key(r):
        canonical = r["cluster"][0] if r["cluster"] else ""
        return (-r["RankScore"], -r["DebtMag"], canonical)

    clusters.sort(key=_cluster_key)

    # ---------------------------------------
    # DEBUG EXPORT: top-ranked cluster subgraph
    # ---------------------------------------
    if clusters:
        top_cluster_nodes = set(clusters[0]["cluster"])
        G_top = G.subgraph(top_cluster_nodes).copy()

        # -------------------------
        # VALIDATION: induced property (top cluster)
        # -------------------------
        expected_top_edges = {
            (u, v)
            for u, v in G.edges()
            if u in G_top.nodes() and v in G_top.nodes()
        }
        assert set(G_top.edges()) == expected_top_edges, \
            f"Top cluster subgraph not properly induced: {snapshot_name}"

        # -------------------------
        # VALIDATION: node integrity
        # -------------------------
        assert set(G_top.nodes()).issubset(set(G.nodes())), \
            f"Top cluster contains invalid nodes: {snapshot_name}"

        # -------------------------
        # VALIDATION: edge bound sanity
        # -------------------------
        n_top = len(G_top.nodes())
        max_edges_top = n_top * n_top
        assert len(G_top.edges()) <= max_edges_top, \
            f"Top cluster edge count invalid: {snapshot_name}"

        graph_summaries.append(
            summarize_graph(G_top, "TopCluster", snapshot_name)
        )

        if top_dir:
            nx.write_gexf(
                G_top,
                os.path.join(top_dir, f"{snapshot_name}_top_cluster.gexf")
            )

    return clusters, graph_summaries
...
def cluster_similarity(C_t, C_prev):
    """
    Overlap-based similarity:
    For each current cluster, find max overlap with any previous cluster.
    Normalize by total current debt-bearing types.
    """
    total = sum(len(c) for c in C_t)
    score = 0
    for c in C_t:
        max_overlap = max(len(set(c) & set(cp)) for cp in C_prev) if C_prev else 0
        score += max_overlap
...
def run_project(project_name, snapshot_prefix, output_root):
    ...
    GEPHI_ROOT = os.path.join(OUTPUT_ROOT, "gephi")
    CALLGRAPH_DIR = os.path.join(GEPHI_ROOT, "callgraphs")
    DEBT_SUBGRAPH_DIR = os.path.join(GEPHI_ROOT, "debt_subgraphs")
    TOP_CLUSTER_DIR = os.path.join(GEPHI_ROOT, "top_clusters")

    os.makedirs(OUTPUT_ROOT, exist_ok=True)
    os.makedirs(GEPHI_ROOT, exist_ok=True)
    os.makedirs(CALLGRAPH_DIR, exist_ok=True)
    os.makedirs(DEBT_SUBGRAPH_DIR, exist_ok=True)
    os.makedirs(TOP_CLUSTER_DIR, exist_ok=True)
    ...
    for snapshot in snapshots:
        ...
        results, graph_summaries = analyze_snapshot(
            ...
            debt_dir=DEBT_SUBGRAPH_DIR,
            top_dir=TOP_CLUSTER_DIR
        )

        reader_output[snapshot] = results

        clusters = [r["cluster"] for r in results]
        debt_types = set(t for r in results for t in r["cluster"])
        debt_mag = [r["DebtMag"] for r in results]
        rank_scores = [r["RankScore"] for r in results]

        all_graph_summaries.extend(graph_summaries)

        total_mag = sum(debt_mag)
        distribution = [dm / total_mag for dm in debt_mag] if total_mag > 0 else []
        ...
        if previous:
            ...
            top_now = set(clusters[0]) if clusters else set()
            top_prev = set(previous["clusters"][0]) if previous["clusters"] else set()
            ...
            cluster_sim = float(cluster_similarity(clusters, previous["clusters"]))
            ...
            consistency_value = (rho + cluster_sim + variance_stability) / 3.0
    ...
    # 1) JSON (reader-facing cluster listing per snapshot)
    with open(os.path.join(OUTPUT_ROOT, "cluster_results.json"), "w", encoding="utf-8") as f:
        json.dump(reader_output, f, indent=2)

    # 2) Cluster Ranking CSV
    ranking_rows = []

    for snapshot in sorted(reader_output.keys()):
        clusters = reader_output[snapshot]

        for rank, cluster_info in enumerate(clusters, start=1):
            ranking_rows.append({
                "snapshot": snapshot,
                "rank": rank,
                "cluster_types": ", ".join(cluster_info["cluster"]),
                "size": cluster_info["size"],

                "DebtMag": cluster_info["DebtMag"],
                "NormDebt": cluster_info["NormDebt"],
                "RankScore": cluster_info["RankScore"],

                "CC_T": cluster_info["metrics"]["CC_T"],
                "WMT_T": cluster_info["metrics"]["WMT_T"],
                "LCOT_T": cluster_info["metrics"]["LCOT_T"],
                "CBT_T": cluster_info["metrics"]["CBT_T"],
                "RFT_T": cluster_info["metrics"]["RFT_T"],
                "ED_T": cluster_info["metrics"]["ED_T"],
                "CY_T": cluster_info["metrics"]["CY_T"],

                "tau_CC_T": cluster_info["thresholds"]["CC_T"],
                "tau_WMT_T": cluster_info["thresholds"]["WMT_T"],
                "tau_LCOT_T": cluster_info["thresholds"]["LCOT_T"],
                "tau_CBT_T": cluster_info["thresholds"]["CBT_T"],
                "tau_RFT_T": cluster_info["thresholds"]["RFT_T"],
                "tau_ED_T": cluster_info["thresholds"]["ED_T"],
                "tau_CY_T": cluster_info["thresholds"]["CY_T"],

                "ex_CC_T": cluster_info["excess"]["CC_T"],
                "ex_WMT_T": cluster_info["excess"]["WMT_T"],
                "ex_LCOT_T": cluster_info["excess"]["LCOT_T"],
                "ex_CBT_T": cluster_info["excess"]["CBT_T"],
                "ex_RFT_T": cluster_info["excess"]["RFT_T"],
                "ex_ED_T": cluster_info["excess"]["ED_T"],
                "ex_CY_T": cluster_info["excess"]["CY_T"]
            })

    with open(os.path.join(OUTPUT_ROOT, "cluster_ranking.csv"),
        ...
        writer = csv.DictWriter(
            ...
            fieldnames=[
                ...
                "CC_T","WMT_T","LCOT_T","CBT_T","RFT_T","ED_T","CY_T",

                "tau_CC_T","tau_WMT_T","tau_LCOT_T","tau_CBT_T","tau_RFT_T","tau_ED_T","tau_CY_T",

                "ex_CC_T","ex_WMT_T","ex_LCOT_T","ex_CBT_T","ex_RFT_T","ex_ED_T","ex_CY_T"
            ]
        )
        writer.writeheader()
    ...
    summary = {
        ...
        "mean_cluster_similarity": float(np.mean(
            [r["cluster_similarity"] for r in consistency_rows]
        )) if consistency_rows else 0.0,
...
def main():
    ...
    base_output = "proposed cluster analysis"
